...
public class OASProxyHttpServer {
...
private final static Logger LOGGER = Logger.getLogger(ProxyHttpServer.class.getCanonicalName());
	...
	private OASHttpStack httpStack;
	 ...
	 */
	public OASProxyHttpServer(CoapServer server) throws IOException {
	 ...
	 *             the socket exception
	 */
	public OASProxyHttpServer(int httpPort) throws IOException {
	
		this.httpStack = new OASHttpStack(httpPort);
		this.httpStack.setRequestHandler(new RequestHandler() {
			public void handleRequest(Request request) {
				OASProxyHttpServer.this.handleRequest(request);
			}
		});
	}
